...
class upcUsedNew:
    ...
    def upcNew(self, upcCode):
            endPointResponse = requests.get('http://search-price-staging-acdnexszocgkssstdqsmrfq4qq.us-east-1.es.amazonaws.com/product_index/_search?q=upc:%d&size=100&pretty' % upcCode) 
            url='http://search-price-staging-acdnexszocgkssstdqsmrfq4qq.us-east-1.es.amazonaws.com/product_index/_search?q=upc:%d&size=100&pretty' % upcCode
            print url
            contentFromEndPoint = json.loads(endPointResponse.content, object_pairs_hook=OrderedDict)
            ...
            for i in range(numberOfResults):
                ...
                for j in range(len(key)):
                    ...
                    if(b[key[j]].get('description')):
                        ...
                        product['product_attributes']['description'] = 'null'
                    productList.append(product)
            print json.dumps(productList,indent=4,sort_keys=False)
                

obj = upcUsedNew()
#obj.upcUsed(759606024612)
obj.upcNew(276300)
#obj.upcNew(13803221619)